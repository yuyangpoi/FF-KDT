...
class CPEG(nn.Module):
    '''
    Convolutional Positional Encoding Generator
    '''
    def __init__(self, dim, k=3, num_heads=8):
        super().__init__()
    ...
    def forward(self, x):
        B, C, _, _ = x.shape
        pos = self.proj(x)
        pos = pos.view(B, self.num_heads, C // self.num_heads, -1)
        return pos
...
class MHSA(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, sr_ratio=1, qkv_bias=False, attn_drop=0., proj_drop=0., norm=None, size=None, pos_encoding=False):
        ...
        if pos_encoding:
            ...
            self.cpeg = CPEG(dim, 3, num_heads)

    def forward(self, x):
        B, C, H, W = x.shape
        q = self.query(x).view(B, self.num_heads, C // self.num_heads, -1)
        if self.sr_ratio > 1:
            x = self.norm(self.sr(x))   # [B, C, H//sr_ratio, W//sr_ratio]
        ...
        if self.pos_encoding:
            ...
            # k = k + pos
            # print('self.rel_h.shape: ', self.rel_h.shape)
            # print('self.rel_w.shape: ', self.rel_w.shape)

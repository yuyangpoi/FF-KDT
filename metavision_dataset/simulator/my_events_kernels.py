...
def fill_voxel_sequence_v2(b, ts, x, y, pol, nbins, target_times, bin_index, voxel_grid, bilinear, split):
    # assert split is False
    # assert bilinear is True
    # if split:
    #     num_bins = nbins//2
    # else:
    #     num_bins = nbins
    ...
    tn = target_times[b, bin_index + 1]
    dt = tn - t0
    # ti_star = ((ts - t0) * num_bins / dt) - 0.5     # -0.5 ~ 9.5
    # lbin = int(math.floor(ti_star))                 # -1 ~ 9
    # lbin = max(lbin, 0)                             # 0 ~ 9
    # lbin = min(lbin, num_bins-1)
    ti_star = ((ts - t0) * (num_bins-1) / dt)       # 0 ~ 9
    lbin = int(math.floor(ti_star))                 # 0 ~ 9
    lbin = max(lbin, 0)                             # 0 ~ 9
    lbin = min(lbin, num_bins-2)                    # 0 ~ 8

    if bilinear:
        rbin = min(lbin + 1, num_bins - 1)          # 1 ~ 9
        left_value = max(0, 1 - abs(lbin - ti_star))
        right_value = 1 - left_value
        # print('lbin, rbin, ti_star, left_value, right_value: ', lbin, rbin, ti_star, left_value, right_value)
...
fill_voxel_sequence_v2_cuda = cuda.jit(device=True)(fill_voxel_sequence_v2)
fill_voxel_sequence_v2_cpu = jit(nopython=True)(fill_voxel_sequence_v2)
exec(format_kernel_strings(
    func_name="voxel_grid_sequence_v2", params="voxel_grid, target_times",
    default_params="bilinear=True, split=False",
    on_event_write="fill_voxel_sequence_v2_{runtime}(b, ts, x, y, pol, nbins, target_times, bin_index, voxel_grid, bilinear, split)",
    on_start="nbins = voxel_grid.shape[2]; t0 = target_times[b, 0]; bin_index = 0",
    documentation="Computes an event cube sequence"))
...
## new adding: EST【pos0, neg0, pos1, neg1...】
def fill_EST_sequence(b, ts, x, y, pol, nbins, target_times, bin_index, EST, bilinear, split):
    num_bins = nbins    # nbins = EST.shape // 2 = 10
    # print(num_bins)

    # fills EST non linear cuts
    while ts > target_times[b, bin_index + 1] and bin_index < len(EST)-1:
        bin_index += 1

    bin_index = min(bin_index, len(EST)-1)
    t0 = target_times[b, bin_index]
    tn = target_times[b, bin_index + 1]
    dt = tn - t0
    ...
    if bilinear:
        ...
        if pol > 0: # positive
            if 0 <= lbin < nbins:
                EST[bin_index, b, lbin*2, y, x] += float(left_value)    # EST.shape: [BPTT_t, B, C, H, W]
            if 0 <= rbin < nbins:
                EST[bin_index, b, rbin*2, y, x] += float(right_value)
        else:       # negative
            if 0 <= lbin < nbins:
                EST[bin_index, b, lbin*2+1, y, x] += float(left_value)    # EST.shape: [BPTT_t, B, C, H, W]
            if 0 <= rbin < nbins:
                EST[bin_index, b, rbin*2+1, y, x] += float(right_value)

    else:
        raise NotImplementedError


fill_EST_sequence_cuda = cuda.jit(device=True)(fill_EST_sequence)
fill_EST_sequence_cpu = jit(nopython=True)(fill_EST_sequence)
exec(format_kernel_strings(
    func_name="EST_sequence", params="EST, target_times",
    default_params="bilinear=True, split=False",
    on_event_write="fill_EST_sequence_{runtime}(b, ts, x, y, pol, nbins, target_times, bin_index, EST, bilinear, split)",
    on_start="nbins = EST.shape[2]//2; t0 = target_times[b, 0]; bin_index = 0",
    documentation="Computes an event EST sequence"))

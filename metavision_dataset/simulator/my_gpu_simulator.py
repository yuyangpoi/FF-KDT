...
from .my_cutoff_kernels import _cuda_kernel_dynamic_moving_average, _cpu_kernel_dynamic_moving_average
from .my_events_kernels import _cuda_kernel_count_events, _cpu_kernel_count_events
from .my_events_kernels import _cpu_kernel_fill_events, _cuda_kernel_fill_events
# from my_events_kernels import _cuda_kernel_voxel_grid_sequence, _cpu_kernel_voxel_grid_sequence
from .my_events_kernels import _cuda_kernel_voxel_grid_sequence_v2, _cpu_kernel_voxel_grid_sequence_v2
from .my_events_kernels import _cuda_kernel_EST_sequence, _cpu_kernel_EST_sequence
...
class GPUEventSimulator(nn.Module):
    ...
    def event_volume_sequence_v2(
        ...
        self._kernel_call(
            log_images, video_len, image_ts, first_times,
            _cuda_kernel_voxel_grid_sequence_v2,
            _cpu_kernel_voxel_grid_sequence_v2,
            args, True, mode == 'bilinear', split_channels, reset_rng_states=True)
    ...
    def event_EST_sequence(
        ...
        # prepare args
        args = [EST, target_timestamps]

        self._kernel_call(
            log_images, video_len, image_ts, first_times,
            _cuda_kernel_EST_sequence,
            _cpu_kernel_EST_sequence,
            args, True, mode == 'bilinear', split_channels, reset_rng_states=True)

        self.prev_image_ts = image_ts[:, -1]

        EST = EST.view(batch_times, batch_size, num_channels, height, width)
        return EST
    ...
    # ## 新增帧间隔EST+曝光时间EST
    # @torch.no_grad()
    # def event_EST_sequence_with_exposure_EST(
    #         self,
    #         log_images,
    #         video_len,
    #         image_ts,
    ...
    #     """
    #     height, width = log_images.shape[:2]
    #     frame_interval_num_channels = frame_interval_nbins * 2 # if split_channels else nbins
    #     exposure_num_channels = exposure_time_nbins * 2
    #     batch_size = len(video_len)
    #     batch_times = target_timestamps.shape[1] - 1
    #
    #     device = log_images.device
    #     frame_interval_EST = torch.zeros(
    #         (batch_times, batch_size, frame_interval_num_channels, height, width),
    #         dtype=torch.float32, device=device)
    #     exposure_interval_EST = torch.zeros(
    #         (batch_times, batch_size, frame_interval_num_channels, height, width),
    #         dtype=torch.float32, device=device)
    #
    #     # arbitrary voxel start times and durations
    ...
    #
    #     # prepare args
    #     args = [frame_interval_EST, target_timestamps]
    #
    #     self._kernel_call(
    #         log_images, video_len, image_ts, first_times,
    #         _cuda_kernel_EST_sequence,
